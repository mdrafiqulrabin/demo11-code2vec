...
TRAIN_DATA_FILE=${MAIN_DIR}/${DATASET_NAME}.train.raw.txt
VAL_DATA_FILE=${MAIN_DIR}/${DATASET_NAME}.val.raw.txt
TEST_DATA_FILE=${MAIN_DIR}/${DATASET_NAME}.test.raw.txt
EXTRACTOR_JAR=JavaExtractor/JPredict/target/JavaExtractor-0.0.1-SNAPSHOT.jar

mkdir -p ${MAIN_DIR}
mkdir -p ${MAIN_DIR}/${DATASET_NAME}
...
TARGET_HISTOGRAM_FILE=${MAIN_DIR}/${DATASET_NAME}/${DATASET_NAME}.histo.tgt.c2v
ORIGIN_HISTOGRAM_FILE=${MAIN_DIR}/${DATASET_NAME}/${DATASET_NAME}.histo.ori.c2v
PATH_HISTOGRAM_FILE=${MAIN_DIR}/${DATASET_NAME}/${DATASET_NAME}.histo.path.c2v
...
${PYTHON} preprocess.py --train_data ${TRAIN_DATA_FILE} --test_data ${TEST_DATA_FILE} --val_data ${VAL_DATA_FILE} \
  ...
  --path_histogram ${PATH_HISTOGRAM_FILE} --target_histogram ${TARGET_HISTOGRAM_FILE} --output_name ${MAIN_DIR}/${DATASET_NAME}/${DATASET_NAME}
